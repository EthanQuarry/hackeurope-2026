...
# --- Iridium SBD Communication Models ---
...
class IridiumGateway(BaseModel):
    name: str
    location: str
    lat: float
    lon: float
...
class ATCommandSequence(BaseModel):
    commands: list[ATCommand]
    total_commands: int
    estimated_duration_ms: int


class SBDPayload(BaseModel):
...
class GatewayRouting(BaseModel):
    selected_gateway: IridiumGateway
    routing_reason: str
    satellite_position: dict[str, float]
    signal_hops: int
    estimated_latency_ms: int
    alternative_gateways: list[IridiumGateway] = []


class CommsTranscription(BaseModel):
